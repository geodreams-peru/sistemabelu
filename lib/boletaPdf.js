...
function generateBoletaPdf(data) {
  ...
  return new Promise((resolve, reject) => {
    try {
      const { emp, ajuste, registros, descansosFechas, valorDia, valorHora, periodo, resumen } = data;
      ...
      for (const item of diasCalendario) {
        const fd = new Date(item.fecha + 'T12:00:00');
        let cells;
        if (item.tipo === 'descanso') cells = [`${fd.getDate()} ${DIAS[fd.getDay()]}`, '—', '—', 'Descanso'];
        else if (item.tipo === 'falta') cells = [`${fd.getDate()} ${DIAS[fd.getDay()]}`, '—', '—', 'Falta'];
        else if (item.tipo === 'futuro') cells = [`${fd.getDate()} ${DIAS[fd.getDay()]}`, '—', '—', '—'];
        else {
          const r = item.reg;
          const est = !r.hora_entrada ? 'F' : r.tarde ? 'T' : 'OK';
          cells = [`${fd.getDate()} ${DIAS[fd.getDay()]}`, r.hora_entrada || '—', r.hora_salida || '—', est];
        }
        drawRow(cells, false);
      }
      doc.y = y + 8;
