...
function findBestDataDir(candidates) {
  // 1) Prioridad máxima: carpeta que contiene asistencia.db
  const direct = candidates.find(dir => fs.existsSync(dir) && hasAsistenciaDb(dir));
  if (direct) return { dir: direct, source: 'direct-asistencia.db' };
