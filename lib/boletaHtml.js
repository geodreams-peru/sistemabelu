...
function renderBoletaHtml(data) {
  ...
  const hoyMedio = new Date();
  hoyMedio.setHours(0, 0, 0, 0);
  const diasCalendario = [];
  for (let d = new Date(fDesde); d <= fHasta; d.setDate(d.getDate() + 1)) {
    const f = d.toISOString().slice(0, 10);
    const reg = registrosPorFecha.get(f);
    const futuro = d > hoyMedio;
    if (reg) diasCalendario.push({ tipo: 'asistencia', fecha: f, reg });
    else if (fechasDescanso.has(f)) diasCalendario.push({ tipo: 'descanso', fecha: f });
    else if (futuro) diasCalendario.push({ tipo: 'futuro', fecha: f });
    else diasCalendario.push({ tipo: 'falta', fecha: f });
  }

  const mkCel = item => {
    if (!item) return '<td colspan="4"></td>';
    const fd = new Date(item.fecha + 'T12:00:00');
    if (item.tipo === 'descanso') {
      return `<td class="cc">${fd.getDate()} ${DIAS[fd.getDay()]}</td>` +
        `<td class="cc cn">—</td>` +
        `<td class="cc cn">—</td>` +
        `<td class="cc cn" style="color:#2f855a;font-weight:700;border-right:1px solid #bbb">Descanso</td>`;
    }
    if (item.tipo === 'falta') {
      return `<td class="cc">${fd.getDate()} ${DIAS[fd.getDay()]}</td>` +
        `<td class="cc cn">—</td>` +
        `<td class="cc cn">—</td>` +
        `<td class="cc cn" style="color:#c53030;font-weight:700;border-right:1px solid #bbb">Falta</td>`;
    }
    if (item.tipo === 'futuro') {
      return `<td class="cc">${fd.getDate()} ${DIAS[fd.getDay()]}</td>` +
        `<td class="cc cn">—</td>` +
        `<td class="cc cn">—</td>` +
        `<td class="cc cn" style="color:#999;border-right:1px solid #bbb">—</td>`;
    }
    const r = item.reg;
  ...
  const mid = Math.ceil(diasCalendario.length / 2);
  const col1 = diasCalendario.slice(0, mid);
  const col2 = diasCalendario.slice(mid);
  let calFilas = '';
